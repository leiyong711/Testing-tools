...
# 下拉框布局
canvas.create_window(895, 225, window=cishu_ac)  # 性能测试‘参数次数’按钮布局
canvas.create_window(895, 410, window=ceshu_ac)  # 性能测试‘测试次数’按钮布局
canvas.create_window(895, 620, window=jg_ac)  # Mokey测试‘时间间隔’按钮布局

# 按钮布局
canvas.create_window(780, 30, window=devicesb)  # 性能测试‘查询设备信息’按钮布局
canvas.create_window(780, 112, window=app)  # 性能测试‘查询设备信息’按钮布局
canvas.create_window(400, 320, window=ks)  # 性能测试‘开始测试’按钮布局
canvas.create_window(400, 490, window=csb)  # 启动时间‘启动测试’按钮布局
canvas.create_window(400, 670, window=monkeyb)  # Monkey‘启动测试’按钮布局

# 文字控件布局
canvas.create_window(80, 15, window=pplabel)  # 品牌文字布局
canvas.create_window(80, 45, window=xhlabel)  # 型号文字布局
canvas.create_window(410, 15, window=bblabel)  # Android版本文字布局
canvas.create_window(410, 45, window=sblabel)  # 设备号文字布局
canvas.create_window(80, 100, window=pathlabel)  # 应用路径文字布局
canvas.create_window(80, 125, window=namelabel)  # 应用名文字布局
canvas.create_window(410, 100, window=appblabel)  # 包名文字布局
canvas.create_window(410, 125, window=appalabel)  # 启动类号文字布局
canvas.create_window(400, 165, window=xn)  # 性能测试文字布局
canvas.create_window(130, 280, window=cpulabel)  # 性能测试‘cpu’文字布局
canvas.create_window(635, 280, window=liullabel)  # 性能测试‘流量’文字布局
canvas.create_window(830, 225, window=cs_label)  # 性能测试‘参数次数’文字布局
canvas.create_window(365, 280, window=ramlabel)  # 性能测试‘内存’文字布局
canvas.create_window(80, 320, window=baolabel)  # 性能测试‘包名：’文字布局
canvas.create_window(400, 370, window=qd)  # ‘启动时间测试’文字布局
canvas.create_window(80, 410, window=qbaolabel)  # 启动时间测试‘测试包名’文字布局
canvas.create_window(450, 410, window=qActivitylabel)  # 启动时间测试‘测试包Activity’文字布局
canvas.create_window(830, 410, window=qnumlabel)  # 启动时间测试‘测试次数’文字布局
canvas.create_window(70, 500, window=qdslabel)  # 启动时间测试‘启动时间展示’文字布局
canvas.create_window(400, 580, window=monkeylabel)  # Monkey测试文字布局
...
# 输入文本布局
canvas.create_window(230, 100, window=path_win)  # ‘应用路径：’输入文本布局
canvas.create_window(230, 125, window=name_win)  # ‘应用名：’输入文本布局
canvas.create_window(230, 320, window=baoi_win)  # 性能测试‘包名：’输入文本布局
canvas.create_window(230, 410, window=qbao_win)  # 启动时间测试‘测试包名：’输入文本布局
canvas.create_window(650, 410, window=qActivity_win)  # 启动时间测试‘测试包Activity：’输入文本布局
...
# 输出文本布局
canvas.create_window(230, 15, window=pp_win)  # 性能测试‘品牌’输出文本布局
canvas.create_window(230, 45, window=xh_win)  # 性能测试‘型号’输出文本布局
canvas.create_window(580, 15, window=bb_win)  # 性能测试‘’输出文本布局
canvas.create_window(580, 45, window=devices_win)  # 性能测试‘设备号’输出文本布局
canvas.create_window(580, 100, window=appb_win)  # ‘应用包名’输出文本布局
canvas.create_window(580, 125, window=appa_win)  # ‘启动类’输出文本布局
canvas.create_window(130, 225, window=cpu_win)  # 性能测试‘cpu’输出文本布局
canvas.create_window(635, 225, window=liul_win)  # 性能测试‘流量’输出文本布局
canvas.create_window(365, 225, window=ram_win)  # 性能测试‘内存’输出文本布局
canvas.create_window(230, 500, window=qds_win)  # 启动测试‘启动时间展示’输出文本布局
